...
(i)
    ...
    - SKIP if X = Y, terminate if X = Y-1, otherwise diverge
...
Proof.
  split; intros.
  - inversion H; subst.
    + apply E_IfFalse; simpl in *; try find_rewrite; auto.
    + apply E_IfTrue; simpl in *; try find_rewrite; auto.
  - inversion H; subst.
    + apply E_IfFalse; try apply negb_true_iff; assumption.
    + apply E_IfTrue; try apply negb_false_iff; assumption.
Qed.
...
Suppose (WHILE b DO c END) / st || st'.  If the loop just ended, i.e. st = st',
then it is clear that cequiv (WHILE b DO c END) SKIP.  Otherwise, it must
be that beval st b = true, which is a contradiction.

Now suppose SKIP / st || st.  Since beval st b = false, it must be that
(WHILE b DO c END) does not enter the loop, and therefore 
...
Proof. 
  split; intros.
  - exfalso. 
    eapply WHILE_true_nonterm; eauto.
  - remember (WHILE BTrue DO SKIP END) as d.
    induction H0; try discriminate; find_inversion.
    + discriminate.
    + inversion H0_; subst.
      auto.
Qed.
...
Ltac apply_cequiv :=
  match goal with
    | [ H : cequiv ?c _ |- ?c / _ || _ ] => apply H
    | [ H : cequiv _ ?c |- ?c / _ || _ ] => apply H
  end.

Ltac rewrite_aequiv :=
  match goal with
    | [ H : aequiv ?a _ |- context [ ?a ] ] => rewrite H
    | [ H : aequiv _ ?a |- context [ ?a ] ] => rewrite <- H
  end.

Ltac rewrite_bequiv :=
  match goal with
    | [ H : bequiv ?b _ |- context [ ?b ] ] => rewrite H
    | [ H : bequiv _ ?b |- context [ ?b ] ] => rewrite <- H
  end.

Ltac try_equiv := apply_cequiv || rewrite_aequiv || (try rewrite_bequiv).  
...
Proof. 
  split; 
    intros;
    inversion H1; subst;
    eapply E_Seq; apply_cequiv; eauto.
Qed.
...
(* Some of my own additional results that aequiv and bequiv are congruence
   relations, which lead to nice powerful congruence solving tactics *)

Ltac cong_tac_1 := unfold aequiv, bequiv, cequiv;
                   intros; simpl;
                   find_higher_order_rewrite;
                   reflexivity.

Ltac cong_tac_2 := unfold aequiv, bequiv, cequiv;
                   intros; simpl;
                   find_higher_order_rewrite;
                   find_higher_order_rewrite;
                   reflexivity.

Lemma APlus_congruence :
  forall a1 a1' a2 a2',
    aequiv a1 a1' ->
    aequiv a2 a2' ->
...
Ltac aequiv_congruence := 
  match goal with
    | [ |- aequiv ?a ?a ] => apply refl_aequiv
    | [ _ : aequiv ?a1 ?a2 |- aequiv ?a2 ?a1 ] => apply sym_aequiv; auto
    | [ _ : aequiv ?a1 ?a2, _ : aequiv ?a2 ?a3 |- aequiv ?a1 ?a3 ] => 
      apply trans_aequiv; aequiv_congruence
    | [ |- aequiv (APlus ?a1 ?a2) (APlus ?a1' ?a2') ] => 
      apply APlus_congruence; aequiv_congruence
    | [ |- aequiv (AMinus ?a1 ?a2) (AMinus ?a1' ?a2') ] => 
      apply AMinus_congruence; aequiv_congruence
    | [ |- aequiv (AMult ?a1 ?a2) (AMult ?a1' ?a2') ] => 
      apply AMult_congruence; aequiv_congruence
    | _ => idtac
  end.

Lemma BEq_congruence :
  forall a1 a2 a1' a2',
    aequiv a1 a1' ->
    aequiv a2 a2' ->
    bequiv (BEq a1 a2) (BEq a1' a2').
Proof.
  cong_tac_2.
Qed.

Lemma BLe_congruence :
  forall a1 a2 a1' a2',
    aequiv a1 a1' ->
    aequiv a2 a2' ->
    bequiv (BLe a1 a2) (BLe a1' a2').
Proof.
  cong_tac_2.
Qed.

Lemma BNot_congruence :
  forall b b',
    bequiv b b' ->
    bequiv (BNot b) (BNot b').
Proof.
  cong_tac_1.
Qed.

Lemma BAnd_congruence : 
  forall b1 b1' b2 b2',
    bequiv b1 b1' ->
    bequiv b2 b2' ->
    bequiv (BAnd b1 b2) (BAnd b1' b2').
Proof.
  cong_tac_2.
Qed.

Ltac bequiv_congruence :=
  match goal with
    | [ |- bequiv ?b ?b ] => apply refl_bequiv
    | [ _ : bequiv ?b1 ?b2 |- bequiv ?b2 ?b1 ] => apply sym_bequiv; auto
    | [ _ : bequiv ?b1 ?b2, _ : bequiv ?b2 ?b3 |- bequiv ?b1 ?b3 ] => 
      apply trans_bequiv; bequiv_congruence
    | [ |- bequiv (BEq ?a1 ?a2) (BEq ?a1' ?a2') ] =>
      apply BEq_congruence; aequiv_congruence
    | [ |- bequiv (BLe ?a1 ?a2) (BLe ?a1' ?a2') ] =>
      apply BLe_congruence; aequiv_congruence
    | [ |- bequiv (BNot ?b) (BNot ?b') ] =>
      apply BNot_congruence; bequiv_congruence
    | [ |- bequiv (BAnd ?b1 ?b2) (BAnd ?b1' ?b2') ] =>
      apply BAnd_congruence; bequiv_congruence
    | _ => idtac
  end.

Ltac cequiv_congruence := 
  match goal with
    | [ |- cequiv ?c ?c ] => apply refl_cequiv
    | [ _ : cequiv ?c1 ?c2 |- cequiv ?c2 ?c1 ] => apply sym_cequiv; auto
    | [ _ : cequiv ?c1 ?c2, _ : cequiv ?c2 ?c3 |- cequiv ?c1 ?c3 ] => 
      apply trans_cequiv; cequiv_congruence
    | [ |- cequiv (?X ::= ?a) (?X' ::= ?a') ] => 
      apply CAss_congruence; aequiv_congruence
    | [ |- cequiv (?c1 ;; ?c2) (?c1' ;; ?c2') ] => 
      apply CSeq_congruence; cequiv_congruence
    | [ |- cequiv (IFB ?b THEN ?c1 ELSE ?c2 FI) (IFB ?b' THEN ?c1' ELSE ?c2' FI) ] =>
      apply CIf_congruence; [ bequiv_congruence | cequiv_congruence | cequiv_congruence ]
    | [ |- cequiv (WHILE ?b DO ?c END) (WHILE ?b' DO ?c' END) ] =>
      apply CWhile_congruence; [ bequiv_congruence | cequiv_congruence ]
    | _ => idtac
  end.
...
Ltac cequiv_congruence :=
  match goal with
    ...
    | [ |- cequiv ?c ?c ] => apply refl_cequiv
    | [ _ : aequiv ?a1 ?a2, _ : aequiv ?a2 ?a3 |- aequiv ?a1 ?a3 ] => 
      apply trans_aequiv; cequiv_congruence
    | [ _ : bequiv ?b1 ?b2, _ : bequiv ?b2 ?b3 |- bequiv ?b1 ?b3 ] => 
      apply trans_bequiv; cequiv_congruence
    | [ _ : cequiv ?c1 ?c2, _ : cequiv ?c2 ?c3 |- cequiv ?c1 ?c3 ] => 
      apply trans_cequiv; cequiv_congruence
    | [ |- cequiv (?X ::= ?a) (?X' ::= ?a') ] => 
      apply CAss_congruence; cequiv_congruence
    | [ |- cequiv (?c1 ;; ?c2) (?c1' ;; ?c2') ] => 
      apply CSeq_congruence; cequiv_congruence
    | [ |- cequiv (IFB ?b THEN ?c1 ELSE ?c2 FI) (IFB ?b' THEN ?c1' ELSE ?c2' FI) ] =>
      apply CIf_congruence; cequiv_congruence
    | [ |- cequiv (WHILE ?b DO ?c END) (WHILE ?b' DO ?c' END) ] =>
      apply CWhile_congruence; cequiv_congruence
    | _ => idtac
  end.
*)
...
Proof.
  ...
  - replace (aeval st a) with (aeval st (fold_constants_aexp a)) by
        (symmetry; apply fold_constants_aexp_sound).
    replace (aeval st a0) with (aeval st (fold_constants_aexp a0)) by
        (symmetry; apply fold_constants_aexp_sound).
    destruct (fold_constants_aexp a);
    destruct (fold_constants_aexp a0);
    try reflexivity.
    destruct (beq_nat n n0) eqn:?; auto.
  - replace (aeval st a) with (aeval st (fold_constants_aexp a)) by
        (symmetry; apply fold_constants_aexp_sound).
    replace (aeval st a0) with (aeval st (fold_constants_aexp a0)) by
        (symmetry; apply fold_constants_aexp_sound).
    ...
    destruct (fold_constants_aexp a0);
    try reflexivity.
    destruct (ble_nat n n0) eqn:?; auto.
  - find_rewrite.
    destruct (fold_constants_bexp b); try reflexivity.
  - repeat find_rewrite.
...
Proof. 
  ...
  Case "WHILE".
    destruct (fold_constants_bexp b) eqn:?;
      try solve [cequiv_congruence; try find_reverse_rewrite; auto using fold_constants_bexp_sound].
    - apply WHILE_true.
...
Proof.
  ...
  induction b; 
    ...
    repeat break_match; 
    bequiv_congruence;
    auto using optimize_0plus_aexp_sound.
Qed.
...
Proof.
  ...
  induction c; 
    simpl;
    cequiv_congruence;
    auto using optimize_0plus_aexp_sound, optimize_0plus_bexp_sound.
Qed.

Definition optimize_0plus_fold_constants (c : com) : com :=
  optimize_0plus_com (fold_constants_com c).

Theorem optimize_0plus_fold_constants_sound :
  ctrans_sound optimize_0plus_fold_constants.
Proof.
  unfold ctrans_sound, optimize_0plus_fold_constants.
  intros.
  eapply trans_cequiv.
...
Proof.
  induction 1; 
    try reflexivity; 
    simpl;
    try (repeat find_rewrite; reflexivity).
  unfold update.
...
Ltac find_ceval_invc :=
  match goal with
    | [ H : ?c / ?st || ?st' |- _ ] => inversion H; subst; clear H
  end.
...
Proof.
  ...
  induction a2; intros; simpl in *; try solve [cequiv_congruence].
  - break_if; try solve [cequiv_congruence].
    split; intros.
    + repeat find_ceval_invc.
      eapply E_Seq; eauto.
      rewrite plus_0_r.
      eapply E_Ass.
      simpl.
      erewrite aeval_weakening; auto.
      unfold update.
      break_if; try solve [exfalso; auto]; auto.
    + repeat find_ceval_invc. simpl.
      eapply E_Seq; eauto.
      rewrite plus_0_r.
      eapply E_Ass.
      simpl.
  ...
  - split; intros.
    + repeat find_ceval_invc.
      ...
      apply E_Ass.
      simpl.
      repeat erewrite aeval_weakening; auto.

  induction a2; intros; inversion H; subst.
  - split; intros;
      inversion H0;
      inversion H3;
      inversion H6;
  ...
  - remember (i1 ::= a1 ;; i2 ::= a2) as c.
    ...
    eapply E_Seq; eauto.
    apply E_Ass.
    destruct a2; simpl in *; try reflexivity.
    + break_match; subst.
      unfold update.
...
Proof. 
  ...
  split; intros.
  - repeat find_ceval_invc.
    replace (update (update st X n0) Y n) with (update (update st Y n) X n0).
    + apply E_Seq with (st' := update st Y n); apply E_Havoc.
    + apply functional_extensionality.
      intros.
      rewrite update_permute; try reflexivity.
      discriminate.
  - repeat find_ceval_invc.
    replace (update (update st Y n0) X n) with (update (update st X n) Y n0).
    + apply E_Seq with (st' := update st X n); apply E_Havoc.
    + apply functional_extensionality.
      intros.
      rewrite update_permute; try reflexivity.
      discriminate.
...
Proof. 
  ...
  assert (ptwice / empty_state || (update (update empty_state X 1) Y 2)) by
    (apply E_Seq with (st' := update empty_state X 1); apply E_Havoc).
  apply H in H0.
  repeat find_ceval_invc.
  simpl in *.
  rewrite update_eq in *.
  remember (update (update empty_state X n) Y n) as st'.
  assert (st' X = st' Y) by
    (rewrite Heqst';
     rewrite update_eq;
  ...
  assert (st' Y = 2) by
    (rewrite H5, update_eq; reflexivity).
  assert (st' X = 1) by
    (rewrite H5, update_permute, update_eq;
       auto;
       unfold not; discriminate).
  congruence.
Qed.
...
Proof. 
  unfold not. intros.
  remember p1 as c.
  induction H0; try discriminate; unfold p1 in *; find_inversion.
  - simpl in *.
    apply H, beq_nat_true, negb_false_iff.
    assumption.
  - inversion H0_; subst.
    inversion H3; subst.
    inversion H6; subst.
...
Proof.
  unfold not. intros.
  remember p2 as c.
  induction H0; try discriminate; unfold p2 in *; find_inversion.
  - simpl in *.
    apply H, beq_nat_true, negb_false_iff.
    assumption.
  - inversion H0_; subst.
...
Proof. 
  ...
  - destruct (eq_nat_dec (st X) 0).
    + find_ceval_invc.
      ...
      * exfalso.
        simpl in *.
        eapply beq_nat_false; try apply negb_true_iff; eauto.
    + find_apply_lem_hyp p1_may_diverge; auto.
      contradiction.
  - destruct (eq_nat_dec (st X) 0).
    + find_ceval_invc.
      * apply E_WhileEnd; auto.
      * exfalso.
        simpl in *.
        eapply beq_nat_false; try apply negb_true_iff; eauto.
    + find_apply_lem_hyp p2_may_diverge; auto.
      contradiction.
Qed.
...
Lemma p3_counterex : 
(*  p3 / (update empty_state X 1) || (update (update (update (update empty_state X 1) Z 1) X 0) Z 2). *)
  forall (st st' : state),
    st = update empty_state X 1 ->
    st' = update (update (update st Z 1) X 0) Z 2 ->
    p3 / st || st'.
Proof.
  intros.
  apply E_Seq with (st' := update st Z 1).
  - apply E_Ass. reflexivity.
  - apply E_WhileLoop with (st' := st').
    + simpl.
      apply negb_true_iff.
      rewrite H.
      rewrite update_permute; try (unfold not; discriminate).
      rewrite update_eq.
      reflexivity.
    + apply E_Seq with (st':= update (update st Z 1) X 0);
      try rewrite H0;
      apply E_Havoc.
    + apply E_WhileEnd. simpl.
...
Proof. 
  ...
  pose proof (p3_counterex st st'). repeat concludes.
  apply H in H0.
  find_ceval_invc.
  find_ceval_invc.
  simpl in *.
  remember (update st'0 Z 1 Z) as n.
  assert (n = 1) by (find_rewrite; rewrite update_eq; reflexivity).
  rewrite H5 in Heqn.
  rewrite update_eq in Heqn.
  congruence.
Qed.
...
Inductive only_mutates (X : id) : com -> Prop :=
  | om_Skip  : only_mutates X SKIP
  | om_Ass   : forall a, only_mutates X (X ::= a)
  | om_Seq   : forall c1 c2,
                 only_mutates X c1 ->
                 only_mutates X c2 ->
                 only_mutates X (c1 ;; c2)
  | om_If    : forall b c1 c2,
                 only_mutates X c1 ->
                 only_mutates X c2 ->
                 only_mutates X (IFB b THEN c1 ELSE c2 FI)
  | om_While : forall b c,
                 only_mutates X c ->
                 only_mutates X (WHILE b DO c END)
  | om_Havoc : only_mutates X (HAVOC X).

Ltac crush_update :=
  match goal with
    | [ |- update ?st ?X (?st ?X) = ?st ] => apply functional_extensionality;
                                             intros;
                                             apply update_same;
                                             reflexivity
    | [ |- ?st = update ?st ?X (?st ?X) ] => symmetry; 
                                             crush_update
    | [ |- update (update ?st ?X ?m) ?X ?n = update ?st ?X ?n ] => 
        apply functional_extensionality;
        intros;
        apply update_shadow
    | [ |- update ?st ?X ?n = update (update ?st ?X ?m) ?X ?n ] => symmetry; 
                                                                   crush_update
    | [ |- update ?st ?X ?n ?X = ?n ] => apply update_eq
    | [ |- ?n = update ?st ?X ?n ?X ] => symmetry; 
                                         crush_update
    | [ |- update ?st ?X ?n ?Y = ?st ?X ] => apply update_neq;
                                             crush_update
    | [ |- ?st ?X = update ?st ?X ?n ?Y ] => symmetry;
                                             crush_update
    | [ H : ?X <> ?Y |- ?Y <> ?X ] => unfold not;
                                      intros;
                                      apply H;
                                      symmetry;
                                      assumption
    | [ H : ?X <> ?Y |- ?X <> ?Y ] => assumption
  end.

Ltac find_om_invc :=
  match goal with
    | [ H : only_mutates _ _ |- _ ] => inversion H; clear H; subst
  end.
...
Ltac squeeze_bool :=
  match goal with
    | [ H : negb _ = false |- _ ] => apply negb_false_iff in H; squeeze_bool
    | [ H : negb _ = true |- _ ] => apply negb_true_iff in H; squeeze_bool
    | [ H : beq_nat _ _ = true |- _ ] => apply beq_nat_true in H
  end.

Theorem exec_only_mutates_X :
  forall X c st st',
    only_mutates X c -> 
    c / st || st' ->
    exists n,
...
Proof.
  intros.
  induction H0; find_om_invc; repeat concludes; repeat break_exists; subst; try solve [eexists; reflexivity].
  - exists (st X).
    crush_update.
  - exists x.
    crush_update.
  - exists (st X).
    crush_update.
  - assert (only_mutates X (WHILE b1 DO c1 END)) by
      (apply om_While; auto).
    repeat concludes.
    repeat break_exists.
    subst.
...
Theorem while_term_postcond : 
  forall b c st st',
    (WHILE b DO c END) / st || st' ->
    beval st' b = false.
Proof.
  intros.
  remember (WHILE b DO c END) as d.
...
Proof.
  ...
  - inversion H; subst; simpl in *.
    ...
    + replace st' with (update st X 1).
      * apply E_Ass. reflexivity.
      * assert (only_mutates X (WHILE (BNot (BEq (AId X) (ANum 1))) DO HAVOC X END)) 
          by (apply om_While; apply om_Havoc).
        assert (exists n, st' = update st X n)
          by (eapply exec_only_mutates_X; eauto).
        assert (beval st' (BNot (BEq (AId X) (ANum 1))) = false) by
          (eapply while_term_postcond; eauto).
        simpl in *.
        squeeze_bool.
        break_exists.
        rewrite H1 in H4.
        ...
        congruence.
  - inversion H; subst.
    destruct (eq_nat_dec (st X) 1); simpl in *.
    + assert (update st X 1 = st).
      * {
          apply functional_extensionality.
          intros.
          destruct (eq_id_dec X x).
      ...
      * repeat find_rewrite.
        ...
        assumption.
    + apply E_WhileLoop with (st' := update st X 1).
      * simpl.
        apply negb_true_iff.
