...
Ltac find_value_invc :=
  match goal with
    | [ H : bvalue _ |- _ ] => inversion H; clear H; subst
    | [ H : nvalue (tsucc _) |- _ ] => inversion H; clear H; find_value_invc
    | [ H : nvalue _ |- _ ] => inversion H; clear H; subst
    | [ H : value _ |- _ ] => inversion H; clear H; find_value_invc
  end.
...
Ltac find_step_invc :=
  match goal with
    ...
    | [ H : tzero ==> _ |- _ ] => inversion H   
    | [ H : tif _ _ _  ==> _ |- _ ] => inversion H; subst; clear H
    | [ H : tsucc _ ==> _ |- _ ] => inversion H; subst; clear H
    | [ H : tpred _ ==> _ |- _ ] => inversion H; subst; clear H
    | [ H : tiszero _ ==> _ |- _ ] => inversion H; subst; clear H
...
Proof with eauto.
  ...
  step_cases (induction H) Case; intros; find_step_invc;
    try reflexivity;
    try solve [find_step_invc];
    try solve [f_equal; auto];
    try solve [find_step_invc; exfalso; eapply value_is_nf; eauto].
Qed.
...
Ltac find_hastype_invc :=
  match goal with
    | [ H : ||- ttrue \in TNat |- _ ] => inversion H
    | [ H : ||- tfalse \in TNat |- _ ] => inversion H
    | [ H : ||- tzero \in TBool |- _ ] => inversion H
    | [ H : ||- (tsucc _) \in TBool |- _ ] => inversion H
    | [ H : ||- _ \in _ |- _ ] => inversion H; subst; clear H
  end.
...
Proof.
  ...
  find_value_invc.
  + right. apply nv_zero.
  + right. apply nv_succ. auto.
Qed.
*)

Ltac crush_value :=
  match goal with
    | [ _ : (||- ?t \in TNat), _ : (value ?t) |- value (tsucc ?t) ] => right;
                                                                     apply nv_succ;
                                                                  apply nat_canonical;
                                                                  assumption
    | [ _ : (||- ?t \in TNat), _ : (value ?t) |- value (tpred ?t) ] => right;
                                                                     apply nv_succ;
                                                                  apply nat_canonical;
                                                                  assumption
  end.

(*    | [ _ : value (tsucc (tsucc ?t)) |- value ?t ] => apply value_succ;
                                                      crush_value
    | [ _ : value (tsucc ?t) |- value ?t ] => apply value_succ; 
                                              assumption *)

(*    | [ _ : |- ?t \in TNat, _ : value ?t |- value (tsucc (tsucc ?t)) ] => 
        right;
        apply nv_succ;
        apply nv_succ;
        apply nat_canonical;
...
(** _Proof_: By induction on a derivation of [||- t \in T].
      ...
      - If the last rule is [T_Succ], then [t = tsucc t1], [T = TNat], and 
        suppose that the theorem holds for when [||- t1 \in TNat].  If 
        [value t1], then we have [nvalue t1] by [nat_canonical], and therefore.
        Then we can conclude [value (tsucc t1)] by showing [nvalue (tsucc t1)],
        which in turn follows from [nv_succ].  Otherwise, if [t1] can
        step to some [t1'], then we know by the [ST_Succ] rule that
        [tsucc t1] can step to some [tsucc t1'].

      - If the last rule is [T_Pred], then [t = tpred t1], [T = TNat], and
        suppose that the theorem holds for when [||- t1 \in TNat].  It suffices
        to show that [pred t1] steps to some [t1'].  If [value t1], then
        we have by [nat_canonical] that [nvalue t1].  If [t1 = tzero], then
        we have that [pred t1] can step to [tzero] by [ST_PredZero].  If
        [t1 = tsucc t2] for some [t2], then we have that [pred t1] can step to
        [t2] by [ST_PredSucc].  If [t1] is not a value and instead can step
        to some [t2], then we have that [pred t1] can step to [pred t2] by
        [ST_Pred].
...
Proof with eauto.
  intros.
  generalize dependent T.
  induction H0; intros; find_hastype_invc; auto.
  find_hastype_invc. auto.
Qed.
...
(** Having seen the subject reduction property, it is reasonable to
    ...
    No.  Consider the ill-typed expression [tif ttrue tzero tfalse].
    This expression steps by [ST_IfTrue] to [tzero], which has type
    [TNat].  However, the original expression has no type.
...
(** Suppose, that we add this new rule to the typing relation: 
   ...
   counterexample.
      ...
      - Progress   becomes false.  [tsucc ttrue] has type [TBool], but
        it is neither a value nor can step to another term.
...
(** Suppose, instead, that we add this new rule to the [step] relation: 
   ...
   - determinism : We have [(tif ttrue t2 t3) ==> t2] by [ST_IfTrue] and
...
(** Suppose instead that we add this rule:
   ...
   - determinism : Suppose [t1 ==> t1'] and [t2 ==> t2'].  Then we have
                   [(tif t1 t2 t3) ==> (tif t1' t2 t3)] by [ST_If] and
                   [(tif t1 t2 t3) ==> (tif t1 t2' t3)] by [ST_Funny2].
...
(** Suppose instead that we add this rule:
   ...
   No properties become false here.  The tpred-false branch does not
   merge with any other step branch, so step is still deterministic.
   Of the new reachable tpred-tfalse terms, these can always step
   to a new term, so progress still holds.  And the new tpred-false
   terms are ill-typed, and only ill-typed terms can step to it, so
   preservation also vacuously applies.  But this rule is weird in that
   step is no longer normalizing, which we haven't shown here but
   should be true.
...
(** Suppose instead that we add this rule:
   ...
   - progress : [(tif tzero t2 t3)] is neither a value nor can step to
     another term.
...
(** Suppose instead that we add this rule:
   ...
   - progress: [(tif (tpred tzero) t2 t3)] steps by [ST_If] to 
     [(tif tzero t2 t3)] since [(tpred tzero) ==> tzero] by [ST_PredZero],
     but [(tif tzero t2 t3)] is neither a value nor can step to another
     term.

   - preservation : [tpred tzero] has type [TBool] and steps by [ST_PredZero]
     to [tzero], which has type [TNat].
...
(** The evaluation rule [E_PredZero] is a bit counter-intuitive: we
    ...
    We no longer have the progress property, since [tpred tzero] would
    neither be a value nor would be able to make a step.
[] *)
...
(** Suppose our evaluation relation is defined in the big-step style.
    ...
    - progress : ||- t \in T -> value (teval t)

    - preservation : ||- t \in T -> ||- (teval t) \in T
[]
*)
