...
(** _Technical note_: Substitution becomes trickier to define if we
    ...
    variables that are not bound by some enclosing lambda), we can skip
...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop := 
  ...
  | s_var2  : forall y, 
                y <> x -> substi s x (tvar y) (tvar y)
  | s_abs1  : forall T t1,
                substi s x (tabs x T t1) (tabs x T t1)
  | s_abs2  : forall y t1 t1' T,
                y <> x -> 
                substi s x t1 t1' ->
...
Ltac crush_hastype :=
  match goal with
    | [ |- (_ ||- tabs _ _ _ \in _) ] => apply T_Abs; crush_hastype
    | [ |- (_ ||- tapp _ _ \in _) ] => eapply T_App; crush_hastype
    | [ |- (_ ||- tvar _ \in _) ] => apply T_Var; crush_hastype
    | [ |- extend _ ?x _ ?x = _ ] => apply extend_eq
    | [ |- extend _ ?x _ ?y = _ ] => rewrite extend_neq; 
                                     [ crush_hastype | discriminate ]
  end.
