...
(* counter-ex: {{ True }} X ::= X + 1 {{ X = X + 1 }} is not a valid triple. *)
...
Proof.
  eapply hoare_seq.
  eapply hoare_asgn.
  eapply hoare_consequence with (Q' := fun st => st X = 1).
  eapply hoare_asgn.
  unfold assert_implies. intros. 
...
Ltac simpl_update :=
  match goal with
    | [ |- context [ update ?st ?X ?n ?X ] ] => rewrite update_eq; 
                                                simpl; 
                                                simpl_update
    | [ |- context [ update ?st ?X ?n ?Y ] ] => rewrite update_neq; 
                                                simpl;
                                                [ simpl_update 
...
(* 
Suppose a = (AId X) and X = 2, i.e. n = 2, in some initial start state.
Then X is assigned the value 3 and Y is also assigned the value 3 in
the final state, which does not satisfying that Y = n = 2.
*)
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    eapply hoare_asgn.
    unfold assn_sub, assert_implies. intros.
    simpl_update.
    apply stupid.
  - eapply hoare_consequence_pre.
    eapply hoare_asgn.
...
Ltac simpl_bool :=
  match goal with
    ...
    | [ H : beq_nat _ _ = true |- _ ] => apply beq_nat_true in H;
                                         simpl in H; simpl_bool
    | [ H : ?b = true -> False |- _ ] => apply not_true_is_false in H; 
                                         simpl in H; simpl_bool
    | [ H : ?b = false -> False |- _ ] => apply not_false_is_true; 
                                          simpl in H; simpl_bool
    | _ => idtac
  end.
...
Theorem hoare_repeat :
  forall P Q b c,
    {{P}} c {{Q}} ->
    {{fun st => Q st /\ bassn (BNot b) st}} c {{Q}} ->
    {{P}} REPEAT c UNTIL b END {{fun st => Q st /\ ~ bassn (BNot b) st}}.
Proof.
  unfold hoare_triple in *. intros.
  pose proof (hoare_while Q (BNot b) c H0).
