...
Proof.
  intros. unfold beq_nat in H. destruct n.
  - reflexivity.
  - inversion H.
Qed.
...
Proof.
  ...
  - intros. destruct n.
    + inversion H.
    + simpl in *. inversion H. rewrite H1. apply IHt. apply H1.
Qed.
...
Lemma app_length_cons_useful : forall (X : Type) (l1 l2 : list X) (x : X),
  length (l1 ++ (x :: l2)) = S (length (l1 ++ l2)).
Proof.
  intros X l1. induction l1 as [| h1 t1].
  - reflexivity.
  - intros. simpl. rewrite IHt1. reflexivity.
Qed.

Lemma app_length_comm : forall (X : Type) (l1 l2 : list X),
  length (l1 ++ l2) = length (l2 ++ l1).
Proof.
  intros. generalize dependent l2.
  induction l1 as [| h1 t1].
  - intros. rewrite app_nil. reflexivity.
  - intros. 
Abort.
...
Proof.
  ...
  - intros. destruct n.
    ...
    + simpl in *.
      ...
      rewrite H1.
      apply IHt in H1.
      rewrite app_length_cons_useful.
      rewrite H1.
      rewrite plus_n_Sm.
      reflexivity.
Qed.
...
Lemma split_fst_snd : forall X Y (l : list (X * Y)),
  split l = (fst (split l), snd (split l)).
Proof.
  intros. induction l as [| (x,y) tl]; reflexivity.
Qed.
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l. induction l as [| (x,y) tl ].
  - intros.
    inversion H.
    reflexivity.
  - intros. 
...
Lemma eq_beq_nat : forall (m n : nat),
