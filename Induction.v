...
Lemma evenb_n_SSn : forall n : nat,
  evenb n = evenb (S (S n)).
Proof.
  intros.
  destruct n.
...
Proof.
  ...
  - Case "n = 0".
    reflexivity.
  - Case "n = S n'".
    rewrite <- evenb_n_SSn. rewrite -> IHn'. rewrite -> negb_involutive. reflexivity.
Qed.
...
Theorem commutativity :
  forall (b : bin), convert_b_u (inc b) = S (convert_b_u b).
Proof.
  intros. induction b as [| b' | b'].
  - reflexivity.
  - reflexivity.
  - assert (H: inc (P b') = T (inc b')).
      reflexivity.
    rewrite -> H. simpl. 
    rewrite -> IHb'.
    rewrite -> plus_n_Sm.
    assert (H0: convert_b_u b' + S (convert_b_u b') = S (convert_b_u b') + convert_b_u b').

      rewrite -> plus_comm. reflexivity.
    rewrite -> H0. rewrite -> plus_n_Sm. reflexivity.
Qed.
...
Fixpoint convert_u_b (n : nat) : bin :=
match n with
  | O    => Z
  | S n' => inc (convert_u_b n')
end.

Eval compute in (convert_u_b (S(S(S(S(S(S O))))))).

Theorem convert_u_b_u__id : 
  forall (n : nat), convert_b_u (convert_u_b n) = n.
Proof.
  intros. induction n as [| n'].
  - reflexivity.
  - simpl. rewrite -> commutativity. rewrite -> IHn'. reflexivity.
Qed.

(* (b) *)

Eval compute in (convert_u_b (convert_b_u (P (P (T Z))) ) ).
