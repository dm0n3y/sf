...
Proof.
  intros.
  apply hoare_consequence with (P' := fun st => parity (st X) = parity m)
                               (Q' := fun st => parity (st X) = parity m /\
                                                ~ bassn (BLe (ANum 2) (AId X)) st).
  - apply hoare_while.
    unfold hoare_triple, bassn. intros.
    inversion H; subst. simpl in *.
    intuition. 
    repeat break_match; try discriminate.
    rewrite update_eq.
    rewrite parity_ge_2; omega.
  - congruence.
  - unfold assert_implies, bassn. intros.
    intuition. simpl in *.
    break_match; auto.
    break_match; auto.
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    ...
    simpl_update.
    omega.
  - unfold assert_implies, assn_sub, hoare_triple.
    intros.
    assert ((X ::= APlus (AId Y) (ANum 1)) / st || update st X (aeval st (APlus (AId Y) (ANum 1)))) by (apply E_Ass; reflexivity).
    find_apply_hyp_hyp.
...
Proof.
  ...
  assert ((HAVOC X) / st || update st X n) by (apply E_Havoc).
  eapply H.
  eapply E_Havoc.
  auto.
Qed.
...
Example slow_assignment_dec (m:nat) : dcom := (
  ...
  WHILE BNot (BEq (AId X) (ANum 0))
  DO
    {{ fun st => (st X) + (st Y) = m /\ bassn (BNot (BEq (AId X) (ANum 0))) st }} ->>
...
Example factorial_dec (m : nat) : dcom := (
  ...
  WHILE BNot (BEq (AId X) (ANum 0))
  DO
    {{ fun st => (st Y) * (real_fact (st X)) = real_fact m /\ bassn (BNot (BEq (AId X) (ANum 0))) st }} ->>
...
Theorem factorial_dec_correct :
  forall m, dec_correct (factorial_dec m).
Proof.
  intros.
  verify.
  - rewrite mult_assoc_reverse.
    replace (st X * real_fact (st X - 1)) with (real_fact (st X)); auto.
