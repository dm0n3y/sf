...
Ltac find_step_invc :=
  match goal with
    | [ H : step ?t ?t' |- _ ] => inversion H; subst; clear H
  end.

Ltac find_value_invc := 
  match goal with
    | [ H : value ?v |- _ ] => inversion H; subst; clear H
  end.
...
Proof.
  ...
  - find_step_invc.
    + find_step_invc.
    + erewrite IHstep; eauto.
    + find_value_invc.
      find_step_invc.
  - find_value_invc.
    find_step_invc; try solve [find_step_invc].
    erewrite IHstep; eauto.
Qed.
...
Proof.  
  ...
  - right.
    destruct IHt1.
    + destruct t1. 
      eexists. eapply ST_IfTrue.
      eexists. eapply ST_IfFalse.
      inversion H.
    + destruct H.
      eexists.
      eapply ST_If.
...
(** It can be shown that the determinism and strong progress theorems
    ...
    - Is the [step] relation still deterministic?  Write yes or no and
      ...
      No.  Consider tif (tif ttrue ttrue ttrue) ttrue ttrue.  By ST_If, this
      steps to tif ttrue ttrue ttrue.  By ST_ShortCircuit, this steps to ttrue.
...
(**
   - Does a strong progress theorem hold? Write yes or no and
     ...
     Yes.  The additional step rule only provides an additonal step option
     for non-value terms, and does not step them into non-value stuck states.
...
(**
   - In general, is there any way we could cause strong progress to
     ...
     Yes.  Take away ST_IfTrue.  Then if a guard eventually steps to 
     ttrue, then we are stuck.
...
Proof.
  ...
  - concludes.
    ...
    + replace y0 with y in *. auto.
      apply (step_deterministic x y y0); auto.
Qed.
...
Proof.
  ...
  - eapply multi_trans.
    ...
    eapply multi_trans.
    apply multistep_congr_2; eauto; try apply v_const.
    apply multi_R.
    apply ST_PlusConstConst.
Qed.
...
Proceed by induction on [ t || n ].

Suppose in the base case that [ t = C n ].  Then we have that [ t ==>* C n ]
by the fact that [ ==>* ] is a reflexive relation.

Now suppose that [ t = P t1 t2 ], [ n = n1 + n2 ], and we have that 
[ t1 || n1 -> t2 ==>* C n1 ] and [ t2 || n2 -> t2 ==>* C n2 ].  To show 
that [ P t1 t2 ==>* C (n1 + n2) ], it suffices by transitivity to show
that [ P t1 t2 ==>* P (C n1) (C n2) ] and [ P (C n1) (C n2) ==>* C (n1 + n2) ].
The latter follows by the fact that [ ==>* ] contains [ ==> ] and the
[ ST_PlusConstConst ] constructor.  The former follows by applications of
our congruence lemmas [ multistep_congr_1 ] and [ multistep_congr_2 ].
...
Proof.
  ...
  intros. intuition.
  find_apply_lem_hyp nf_same_as_value.
  find_value_invc.
  exists n.
...
Ltac find_value_invc :=
  match goal with 
    | [ H : value ?t |- _ ] => inversion H; subst; clear H
  end.

Ltac find_step_invc :=
  match goal with 
    | [ H : (C ?n) ==> ?t |- _ ] => inversion H
    | [ H : ttrue ==> ?t |- _ ] => inversion H
    | [ H : tfalse ==> ?t |- _ ] => inversion H
    | [ H : ?t ==> ?t' |- _ ] => inversion H; subst; clear H
  end.
...
Proof.
  ...
  - find_step_invc; try solve [find_step_invc].
    reflexivity.
  - inversion H0; subst; try find_value_invc; try solve [find_step_invc].
    rewrite (IHstep t1'0); auto.
  - inversion H1; subst; try find_value_invc; 
      try solve [find_step_invc];
      f_equal; auto.
  - find_step_invc. reflexivity. find_step_invc.
  - find_step_invc. reflexivity. find_step_invc.
  - inversion H0; subst; try solve [find_step_invc].
    f_equal. auto.
Qed.
...
(*  | AS_Plus2 : forall n st a2 a2',
                 a2 / st ==>a a2' ->
                 (APlus (ANum n) a2) / st ==>a (APlus (ANum n) a2') *)
...
Theorem CS_Seq_congruence : 
  forall c1 c1' c2 st st',
    c1 / st ==>* c1' / st' ->
    (c1 ;; c2 ) / st ==>* (c1' ;; c2) / st'.
Proof.
  intros.
  ...
  - inversion Heqx'; subst.
    apply multi_refl.
  - apply multi_step with (y0 := ( (fst y) ;; c2 , snd y)).
    + apply CS_SeqStep.
      replace (fst y, snd y) with y;
...
Proof.
  ...
  - inversion Heqx'; subst.
    apply multi_refl.
  - apply multi_step with (y0 := (PAR (fst y) WITH c2 END, snd y)).
    + apply CS_Par1.
      replace (fst y, snd y) with y;
      auto using surjective_pairing.
    + apply IHmulti;
...
Proof.
  ...
  - inversion Heqx'; subst.
    apply multi_refl.
  - apply multi_step with (y0 := (PAR c1 WITH (fst y) END, snd y)).
    + apply CS_Par2.
      replace (fst y, snd y) with y;
      auto using surjective_pairing.
    + apply IHmulti;
...
Proof.
  intuition.
  unfold par_loop.
  apply CS_Par2_congruence.
  eapply multi_step. apply CS_While.
  eapply multi_step. apply CS_IfStep.
    apply BS_Eq1. apply AS_Id.
  eapply multi_step. apply CS_IfStep.
    apply BS_Eq. find_rewrite. simpl.
  eapply multi_step. apply CS_IfTrue.
  ...
  find_rewrite. rewrite plus_comm.
  apply multi_refl.
Qed.
...
Ltac find_stackmultistep_invc :=
  match goal with
    | [ H : stack_multistep ?st (?i :: ?p, ?stk) (?p', ?stk') |- _ ] => inversion H; subst; clear H
    | [ H : stack_multistep ?st (?p, ?stk) (?p', ?stk') |- _ ] => inversion H; subst; clear H
  end.

Ltac find_stackstep_invc :=
  match goal with
    | [ H : stack_step ?st (?i :: ?p, ?stk) ?x |- _ ]  => inversion H; subst; clear H
  end.

Lemma stack_step_app : 
  forall (p1 p2 : prog) (stk1 stk2 stk3 : stack) (st : state),
...
Proof.
  ...
  - inversion H; subst.
    + auto.
    + inversion H1.
  - Hint Constructors multi stack_step.
    destruct a; 
      find_stackmultistep_invc; 
      find_stackstep_invc;
      eapply multi_step;
      eauto;
      eapply IHp1; eauto.
...
Theorem compiler_is_correct' :
  forall (e : aexp) (st : state) (stk : stack),
    stack_multistep st (s_compile e, stk) ([], (aeval st e) :: stk).
Proof.
  induction e; intros; simpl.
  - apply multi_R, SS_Push.
  - apply multi_R, SS_Load.
  - eapply stack_step_app; eauto.
    eapply stack_step_app; eauto.
    apply multi_R, SS_Plus.
  - eapply stack_step_app; eauto.
    eapply stack_step_app; eauto.
    apply multi_R, SS_Minus.
  - eapply stack_step_app; eauto.
    eapply stack_step_app; eauto.
    apply multi_R, SS_Mult.
Qed.
