...
Fixpoint optimize_a (a : aexp) : aexp := 
  match a with
    | ANum n       => ANum n
    | APlus a1 a2  => match a1, a2 with
                        | (ANum 0), (ANum 0) => ANum 0
                        | (ANum 0), _        => optimize_a a2
                        | _, (ANum 0)        => optimize_a a1
                        | _, _               => APlus (optimize_a a1) (optimize_a a2)
                      end
    | AMinus a1 a2 => match a1, a2 with
                        | (ANum 0), _  => ANum 0
                        | _, (ANum 0)  => optimize_a a1
                        | _, _         => AMinus (optimize_a a1) (optimize_a a2)
                      end
    | AMult a1 a2  => match a1, a2 with
                        | (ANum 0), _ => ANum 0
                        | _, (ANum 0) => ANum 0
                        | (ANum 1), _ => optimize_a a2
                        | _, (ANum 1) => optimize_a a1
                        | _, _        => AMult (optimize_a a1) (optimize_a a2)
                      end
  end.
...
Fixpoint optimize_b (b : bexp) : bexp :=
  match b with 
    ...
    | BNot b1     => match b1 with
                       | BFalse => BTrue
                       | BTrue  => BFalse
                       | _      => BNot (optimize_b b1)
                     end
    | BAnd b1 b2  => match b1, b2 with
                       | BFalse, _    => BFalse
                       | BTrue, BTrue => BTrue
                       | _, _         => BAnd (optimize_b b1) (optimize_b b2)
                     end
  end.

Theorem optimize_a_sound : 
  forall (a : aexp),
...
Proof.
  induction b;
    simpl;
    try reflexivity;
    repeat (rewrite optimize_a_sound);
    repeat break_match_goal;
    try rewrite <- IHb;
    try rewrite <- IHb1; 
    try rewrite <- IHb2;
...
Proof.
  intros.
  unfold update.
  destruct (eq_id_dec x2 x1); subst.
  - rewrite eq_id.
    reflexivity.
  - apply neq_id. 
    unfold not.
...
Proof.
  ...
  - auto.
  - apply E_Seq with (update (update empty_state X 0) Y 1); auto.
Qed.
(** [] *)
...
(*
Notation "'(' x ',' n ')' :: s" := (update s x n) (at level 60, right associativity).
Notation "[ ]" := empty_state.
Notation "[ ( x , n ) ; .. ; ( y , m ) ]" := ((x,n) :: .. :: (y,m) :: []).
  (update ( .. (update empty_state y m) .. ) x n).
*)
...
Proof.
  Hint Constructors ceval.
  apply E_Seq with (update (update empty_state X 2) Y 0); auto.
  apply E_WhileLoop with (update (update (update (update empty_state X 2) Y 0) Y 2) X 1); auto.
  - apply E_Seq with (update (update (update empty_state X 2) Y 0) Y 2); auto.
  - apply E_WhileLoop with (update (update (update (update (update (update empty_state X 2) Y 0) Y 2) X 1) Y 3) X 0); auto.
    + apply E_Seq with (update (update (update (update (update empty_state X 2) Y 0) Y 2) X 1) Y 3); auto.
Qed.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nwr_ass  : forall id n, no_whilesR (id ::= n)
  | nwr_seq  : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
  | nwr_ifte : forall b c1 c2, 
                 no_whilesR c1 -> 
                 no_whilesR c2 -> 
...
Proof.
  ...
  induction 1; intros.
  - exists st. auto.
  - exists (update st id (aeval st n)). auto.
...
Lemma s_execute_append :
  forall (prog1 prog2 : list sinstr) (st : state) (stack stack': list nat),
...
Proof.
  induction e;
    intros; 
    simpl in *;
    repeat (rewrite s_execute_append by auto);
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Break      : forall st, 
                     BREAK / st || SBreak / st
  | E_Ass        : forall st x a, 
                     (x ::= a) / st || SContinue / (update st x (aeval st a))
  | E_SeqBreak   : forall st st' c1 c2,
                     c1 / st || SBreak / st' ->
                     (c1 ; c2) / st || SBreak / st'
  | E_SeqCont    : forall st st' st'' c1 c2 status,
                     c1 / st || SContinue / st' ->
                     c2 / st' || status / st'' ->
                     (c1 ; c2) / st || status / st''
  | E_IfTrue     : forall st st' b c1 c2 status,
                     beval st b = true ->
                     c1 / st || status / st' -> 
                     (IFB b THEN c1 ELSE c2 FI) / st || status / st'
  | E_IfFalse    : forall st st' b c1 c2 status,
                     beval st b = false ->
                     c2 / st || status / st' ->
                     (IFB b THEN c1 ELSE c2 FI) / st || status / st'
  ...
  | E_WhileCont  : forall st st' st'' b c,
                     beval st b = true ->
                     c / st || SContinue / st' ->
                     (WHILE b DO c END) / st' || SContinue / st'' ->
                     (WHILE b DO c END) / st || SContinue / st'' 
...
Proof.
  ...
  generalize dependent st'.
  induction 1; try discriminate; intros; inversion Heqcc; subst.
  - congruence.
...
Proof.
  ...
  - apply IHceval.
    inversion H0; subst.
    + assumption.
    + destruct (IHceval st'0 SContinue H3).
      discriminate.
  - apply IHceval2.
    inversion H1; subst.
    + destruct (IHceval1 st2 SBreak H7). discriminate.
    + destruct (IHceval1 st'0 SContinue H4); subst.
      assumption.
  - apply IHceval.
    inversion H1; congruence.
  - apply IHceval.
    inversion H1; congruence.
  - inversion H0; subst; split; congruence.
  - inversion H1; subst.
    + congruence.
    + split; destruct (IHceval st2 SBreak H8); auto.
    + destruct (IHceval st'0 SContinue H5). discriminate.
  - inversion H2; subst; try congruence.
    + specialize (IHceval1 st2 SBreak). concludes. break_and. discriminate.
    + specialize (IHceval1 st'0 SContinue). concludes. break_and. subst.      
      specialize (IHceval2 st2 SContinue). concludes. break_and. subst.
      split; reflexivity.
Qed.
